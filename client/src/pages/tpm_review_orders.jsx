...
const ReviewOrderPage = () => {
  ...
  const filteredOrders = torders.filter((order) => {
    if (activeTab === "Rejected") return order.status === "MRejected";
    if (activeTab === "Accepted") return order.status === "Accepted";
    return order.status === "Proceed";
  });

  return (
    <div>
      <div className="font-sans container mx-auto p-8 min-h-screen">
                ...
                >
                 ...
                 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-[700]">
                  ...
                  {order.status === "Proceed" ? (
                      <div>
                        <Link to={`/review-an-ordertpm/${order.tshirt_order_id}`} className="text-black font-semibold hover:underline cursor-pointer text-decoration-none">
                          Review
                        </Link>
                      </div>
                    ) : 
                    ...
                    <button className="text-black font-semibold hover:underline cursor-pointer text-decoration-none">
                       <Link to={`/manager-view-order/${order.tshirt_order_id}`}>View More</Link>
                    </button>
